...
GUIDE = [
    ...
    ("⑤ 先方確認事項 … §7-1（catr.jp）はこちらで「認める」と確定させました。経緯と、", ""),
    ("      残る論点（営業収益の扱い等）を記載しています。", ""),
    ("⑥ 再検証で見直した行 … 検索枠が回復したあとに掛け直して判定が変わった行。", ""),
    ("      「架電不可へ」＝誤って載っていたので取り下げた行。「架電可へ」＝回収できた行。", ""),
    ("", ""),
    ("判定ラベルの意味", "h"),
...
WHY = {"catr_resolve": "§7-1確定により回収",
       "edge_verify": "300〜350億の境界帯を再検証",
       "priority_audit": "未再現の架電可行を優先監査"}
aud = []
for k in sorted(ov, key=int):
    n_ = int(k)
    src = ov[k].get("由来", "").split()[0]
    kept = by[n_]["最終判定"].startswith("架電可")
    aud.append({"行": n_, "取引先名": by[n_]["取引先名"], "法人番号": by[n_]["法人番号"],
                "見直しの結果": "架電可へ" if kept else "架電不可へ",
                "きっかけ": WHY.get(src, src), "最終判定": by[n_]["最終判定"],
                "I列_検証結果": by[n_]["I列_検証結果"], "J列_ソース": by[n_]["J列_ソース"]})
aud.sort(key=lambda x: (x["見直しの結果"] == "架電可へ", x["行"]))
ws = sheet(wb, "再検証で見直した行",
           [("行", "行"), ("取引先名", "取引先名"), ("法人番号", "法人番号"),
            ("見直しの結果", "見直しの結果"), ("きっかけ", "きっかけ"), ("最終判定", "最終判定"),
            ("I列_検証結果", "I列_検証結果"), ("J列_ソース", "J列_ソース")],
           aud, [7, 30, 15, 14, 26, 22, 95, 50])
for r_ in range(2, len(aud) + 2):
    if ws.cell(r_, 4).value == "架電不可へ":
        ws.cell(r_, 4).font = ng
        ws.cell(r_, 6).font = ng
        for c in range(1, 9):
